...
public:
  ...
  bool visit(Row & row) { set_.set(row.get_int(0,false));  return false; }
...
public:
  ...
  void visit(Row & row) { row.set_int(0, i_++); }
};
...
public:
  ...
  bool visit(Row & row) override {
    int pid = row.get_int(0,false);
    int uid = row.get_int(1,false);
    if (uSet.test(uid)) 
      if (!pSet.test(pid)) {
    	pSet.set(pid);
        newProjects.set(pid);
...
public:
  ...
  Linus(size_t idx, NetworkIfc& net, size_t num_nodes): Application(idx, net) { this->num_nodes = num_nodes;}
  ...
  *  projects, and the users added in the previous round. */
  ...
  void merge(Set& set, char const* name, int stage) {
	...
	delete delta;
      ...
      SetWriter writer(set);
      Key k(StrBuff(name).c(stage).c("-").c(node).get()->c_str());
      delete DataFrame::fromVisitor(&k, &kv, "I", writer);
