...
DataFrame* DataFrame::fromFile(const char* fileName, Key* key, KDStore* kv) {
    DataFrame* returning = DataFrame::fromFile(fileName);
    kv->put(key, returning);
    return returning;
}

DataFrame* DataFrame::fromScalarInt(Key* key, KDStore* kv, int num) {
    Schema s("I");
    DataFrame* returning = new DataFrame(s);
    Row r(s);
    r.set_int(0, num);
    kv->put(key, returning);
    return returning;
